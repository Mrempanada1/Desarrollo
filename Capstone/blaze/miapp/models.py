...
# from fcm_django.api.rest import FCMDevice
...
def guardar_perfil_usuario(sender, instance, **kwargs):
    try:
        perfil = Perfil.objects.get(user=instance.user)
        if perfil and perfil != instance:
            print(
                "Advertencia: El perfil ya existe y se está intentando crear uno duplicado.")
        else:
            instance.save()
...
class Notificacion(models.Model):
    ...
    # dispositivo_token = models.CharField(max_length=255)

    # def save(self, *args, **kwargs):
    # super().save(*args, **kwargs)
    # Enviar notificación al crear la notificación
    # enviar_notificacion(self.mensaje, self.dispositivo_token)
...
class Proceso(models.Model):
    ...
    # Relacion muchos a muchos con notificacion
